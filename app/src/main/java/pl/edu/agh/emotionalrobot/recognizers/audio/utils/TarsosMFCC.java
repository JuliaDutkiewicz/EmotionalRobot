...
public class TarsosMFCC {
    ...
    private TarsosFFT fft;
    private int samplesPerFrame;
    private float sampleRate;

    public TarsosMFCC(int samplesPerFrame, int sampleRate){
        this(samplesPerFrame, sampleRate, 216, 30, 133.3334f, ((float)sampleRate)/2f);
    }

    public TarsosMFCC(int samplesPerFrame, float sampleRate, int amountOfCepstrumCoef, int amountOfMelFilters, float lowerFilterFreq, float upperFilterFreq) {
        this.samplesPerFrame = samplesPerFrame;
        this.sampleRate = sampleRate;
        this.amountOfCepstrumCoef = amountOfCepstrumCoef;
        this.amountOfMelFilters = amountOfMelFilters;
        this.fft = new TarsosFFT(samplesPerFrame, new HammingWindow());
    ...
    public float[] process(float[] audioBuffer) {
        ...
        // Magnitude Spectrum
        double bin[] = magnitudeSpectrum(audioFloatBuffer);

        // get Mel Filterbank
        double fbank[] = melFilter(bin, centerFrequencies);
     ...
     */
    public double[] magnitudeSpectrum(float[] frame){
        double magSpectrum[] = new double[frame.length];

        // calculate TarsosFFT for current frame
    ...
    /**
     * calculates the TarsosFFT bin indices<br> calls: none<br> called by:
     * featureExtraction
     *
     */
     ...
     */
    public double[] melFilter(double bin[], int centerFrequencies[]) {
        ...
        double fbank[] = new double[amountOfMelFilters];
     ...
     */
    public float[] cepCoefficients(double f[]){
